...
const ResumeReport = () => {
  ...
  // Resume PDF removed - using existing resume analysis
  ...
  const onCompare = async () => {
    try {
      ...
      // Validate inputs - LinkedIn URL or PDF is required
      if (!linkedinUrl && !linkedinPdfFile) {
        setCompareError("Please enter LinkedIn URL or upload LinkedIn PDF");
        return;
      }

      // Resume data is already available from existing resume analysis (id)

      const token = await getToken();
      const formData = new FormData();
      formData.append("userId", user?.id);

      // Determine mode: existing since we have existing_resume_analysis_id (id)
      formData.append("mode", "existing");

      // Fresh LinkedIn input
      if (linkedinUrl) formData.append("linkedinUrl", linkedinUrl);

      // Add resume source (always existing since we're on resume report page)
      formData.append("existing_resume_analysis_id", id);
      // Resume PDF not needed - using existing resume analysis

      // Add LinkedIn PDF if provided (for fresh mode or override)
      const linkedinPdf = linkedinPdfFile || linkedinPdfRef.current?.files?.[0];
      if (linkedinPdf) {
        formData.append("linkedinPdf", linkedinPdf);
      }

      // Role/JD will be automatically extracted from resumeReport by the server
      // The server will use resumeReport.role_id and resumeReport.role_name if available
  ...
  const handleDownloadPdf = async () => {
    try {
      ...
      const token = await getToken();
      const response = await api.get(`/api/pdf/resume/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      a.href = url;
      a.download = `resume-report-${id}.pdf`;
                ...
                */}
                ...
                >
                  ...
                  <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                      <DialogTitle>Compare with LinkedIn</DialogTitle>
                      <DialogDescription>
                        Provide LinkedIn URL or upload LinkedIn PDF to compare
                        with your resume.
                      </DialogDescription>
                    </DialogHeader>
                    ...
                    <div className="mt-6 space-y-6">
                      ...
                      {/* Info: Role/JD will be extracted from existing Resume analysis */}
                      {resumeReport?.role_name || resumeReport?.role_id ? (
                        <div className="space-y-2 border-t pt-4">
                          <p className="text-xs text-muted-foreground">
                            <strong>Note:</strong> Role and job description will
                            be automatically extracted from your Resume analysis
                            (
                            {resumeReport.role_name ||
                              "Role ID: " + resumeReport.role_id}
                            ).
                          </p>
                        </div>
                      ) : (
                      ...
                      <div className="flex gap-3 pt-4 border-t">
                        ...
                        <LoadingButton
                          ...
                          className="flex-1"
                          disabled={!linkedinUrl && !linkedinPdfFile}
                        >
                          Start Comparison
                        </LoadingButton>
                      </div>

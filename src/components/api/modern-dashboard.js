...
/**
 * Modern Dashboard API service with enhanced features and better UX
 */
export const ModernDashboardAPI = {
  /**
  ...
  async getDashboardAnalytics(token) {
    try {
      const response = await api.get("/api/dashboard/v3/analytics", {
        headers: {
  ...
  async getPerformanceMetrics(token, options = {}) {
    try {
      ...
      const response = await api.get(`/api/dashboard/v3/metrics?${params}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  async getSmartRecommendations(token) {
    try {
      const response = await api.get("/api/dashboard/v3/recommendations", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  async getProgressTracking(token) {
    try {
      const response = await api.get("/api/dashboard/v3/progress", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  async getAchievementBadges(token) {
    try {
      const response = await api.get("/api/dashboard/v3/achievements", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  async getRecentActivity(token, options = {}) {
    try {
      ...
      const response = await api.get(`/api/dashboard/v3/activity?${params}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  async getAIInsights(token) {
    try {
      const response = await api.get("/api/dashboard/v3/insights", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export default ModernDashboardAPI;
